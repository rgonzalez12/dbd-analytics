...
func TestEnhancedRetryLogic(t *testing.T) {
	...
	tests := []struct {
		name                  string
		maxRetries           string // Environment variable value
		httpResponses        []int  // Status codes to return in sequence
		retryAfterHeaders    []string // Retry-After header values
		rateLimitResetHeaders []string // X-RateLimit-Reset header values
		expectRetries        int
		expectSuccess        bool
	}{
		{
			...
			retryAfterHeaders: []string{"1", ""}, // Use 1 second instead of 2 for faster testing
			expectRetries: 1,
			expectSuccess: true,
		},
		{
		...
		},
		{
			...
			maxRetries:    "3",
			httpResponses: []int{429, 200},
			rateLimitResetHeaders: []string{strconv.FormatInt(time.Now().Add(1*time.Second).Unix(), 10), ""}, // 1 second instead of 3
			expectRetries: 1,
			expectSuccess: true,
		},
	}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if requestCount < len(tt.httpResponses) {
					...
					// Fallback to 200 if we run out of configured responses
					w.WriteHeader(200)
					w.Write([]byte(`{"response":{"players":[{"steamid":"123","personaname":"Test"}]}}`))
				}
			}))
			...
			// Create client with test server and faster retry configuration for testing
			client := &Client{
				apiKey: "test_key",
				client: &http.Client{},
				retryConfig: RetryConfig{
					...
					BaseDelay:  50 * time.Millisecond,  // Faster for testing
					MaxDelay:   1 * time.Second,        // Faster for testing
					Multiplier: 2.0,
					Jitter:     false, // Disabled for predictable testing
				},
			}

			start := time.Now()
			
			// Test makeRequest directly (which includes retry logic)
			...
			// Verify retry count
			actualRetries := requestCount - 1 // Subtract initial request
			...
			// For rate limit tests, verify some delay occurred (adjust for test timing)
...
func TestExponentialBackoff(t *testing.T) {
	...
	tests := []struct {
		...
		{0, 100 * time.Millisecond, 100 * time.Millisecond}, // 100ms
		{1, 200 * time.Millisecond, 200 * time.Millisecond}, // 200ms
		{2, 400 * time.Millisecond, 400 * time.Millisecond}, // 400ms
		{3, 800 * time.Millisecond, 800 * time.Millisecond}, // 800ms
		{4, 1600 * time.Millisecond, 1600 * time.Millisecond}, // 1.6s
...
func TestRateLimitHeaderParsing(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Allow some tolerance for X-RateLimit-Reset timing
			tolerance := 2
