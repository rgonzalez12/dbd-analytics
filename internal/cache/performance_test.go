...
// TestValidatorPerformance tests the dedicated validator performance
...
// BenchmarkValidatorVsBuiltIn compares validator vs built-in corruption detection
func BenchmarkValidatorVsBuiltIn(b *testing.B) {
	cache := NewMemoryCache(MemoryCacheConfig{
		MaxEntries:      5000,
		DefaultTTL:      5 * time.Minute,
		CleanupInterval: 1 * time.Minute,
